...
int main()
{
  ...
  //  cin >> t;
    while(t--){
        ll n, q;
        cin >> n >> q;
        vector<ll>warrior_powers(n);
        int sum_warriors = 0;
        f(i, n) {
            cin >> warrior_powers[i];
            sum_warriors += warrior_powers[i];
        }
        vector<ll>prefix_sums(n);
        prefix_sums[0] = warrior_powers[0];
        for (int i = 1; i < n; i++)
        {
            prefix_sums[i] = prefix_sums[i - 1] + warrior_powers[i];
        }
        long long power = 0;
        for (int i = 0; i < q; i++)
        {
            ...
            cin >> arrows;
            power += arrows;
            auto currentlyalive = n - (upper_bound(all(prefix_sums), power)-prefix_sums.begin());
            if (currentlyalive == 0)
            {
                currentlyalive = n;
                power = 0;
            }
            cout << currentlyalive << endl;
        }
    }
}