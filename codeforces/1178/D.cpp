    ...
    if (now != 0) {
        for (int mask = 0; mask < (1 << n); mask++) {
            ...
            for (int j = 0; j < n; j++)
                if ((1 << j) & mask)
                    al += (mp[j]);
            if (al != now || !pairup(now - 1, prev | mask)) continue;
            return 1;
        }
        return 0;
    } else return 1;
...
signed main() {
    ...
    ans.push_back({v[0], v.back()});
    for (int i = 0; i < n - 1; i++, lines++)
        ans.push_back({v[i], v[i + 1]});
    for (int i = 0; i < n and !isPrime(lines); i++, lines++)
        ans.push_back({v[i], v[i + n / 2]});
    cout << ans.size() << endl;
    for (auto i: ans)
