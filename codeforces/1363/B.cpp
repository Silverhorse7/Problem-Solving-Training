...
signed main() {
    T {
        string s;
        cin >> s;
        //final string is one of : 100000 000001  0111111 11111110 00000 111111
        vector<string> v;
        auto get_diff = [&](string t) {
            int cnt = 0;
        ...
        for (int i = 0; i < s.size(); i++) {
            v.push_back(string(s.size() - i, '0') + string(i, '1'));
            v.push_back(string(s.size() - i, '1') + string(i, '0'));
        }
        int X = INT_MAX;
        for (auto i: v)
            X = min(X, get_diff(i));
