...
void it_was_me_dio_hahahahahaha()//u will get the largest number in x , and then u will get its binary 
// equilavent , then u will fill all other strings with zero till the size are same as biggest one
// and u will compare as normal again
// oh , and change the parameters of btsetdiff 
{	
	ll  n, m, k;
	cin >> n >> m >> k;
	vector<ll>x(m + 1);
	...
	vector<string>s(m);
	string mix = dec_to_bin_str(*max_element(all(x)));
	string fedor = dec_to_bin_str(x[m]);

	while (fedor.size() < mix.size())
	{
		fedor += '0';
	}
	reverse(all(fedor));
	...
	for (int i = 0; i < m; i++)
	{
		s[i] = dec_to_bin_str(x[i]);

		while (s[i].size() < mix.size())
		{
			s[i] += '0';
		}
		reverse(all(s[i]));
		if (btsetdiff(s[i], fedor) <= k)
		{
			total++;
		}
	}
