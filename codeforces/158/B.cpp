	...
	void it_is_me_dio_hahahahahaha()
	{
		...
		//cout << v1.size() << " " << v2.size() << " " << v3.size() << endl; return;
		while (one>0 and three > 0)// done 
		{
			one--;
			three--;
			result++;
		...
		if (two%2==0 and two>=2) // done
		{
			result += two / 2;
			two = 0;
		}
		if (one % 4 == 0 and one>=4)// done
		{
			result += (one / 4);
			one = 0;
		}
		if (two % 2 != 0 and two > 2)
		{
			result += (two / 2);
			two %= 2;
		}
		if (one % 4 != 0 and one > 4)
		{
			result += (one / 4);
			one %= 4;
		}
		if (two)// two = 1 , one =3,2,1,0 
		{
			if (one==3)
			{
				result++;
