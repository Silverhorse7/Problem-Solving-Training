...
const ll maxk = 2000000000000000000;

string s = "What are you doing at the end of the world? Are you busy? Will you save us?";
string a = R"(What are you doing while sending "x"? Are you busy? Will you send "x"?)";
ll Q, N, K, l[200010];
...
char getans(ll n, ll k) {
    ...
    for (i = 0; i < a.length(); i++) {
        if (a[i] == 'x') {
            if (l[n - 1] >= k) return getans(n - 1, k);
            else k -= l[n - 1];
        } else {
            k--;
            if (k == 0) return a[i];
