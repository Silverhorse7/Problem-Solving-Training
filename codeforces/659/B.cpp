...
signed main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        int team, score;
        cin >> team >> score;
        M[team].push_back({score, s});
    }
    for (auto &i: M)
        sort(all(i.second), greater<pair<int, string>>());
    for (auto i: M) {
        ...
        if (i.second.size() > 2 and i.second[1].first == i.second[2].first)cout << "?\n";
        else
            for (int i = 0; i < output.size(); i++)
                cout << output[i].second << (i < output.size() - 1 ? ' ' : '\n');
    }
}