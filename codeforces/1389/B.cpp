...
int maxScore(int n, int curr_pos, int k, bool flag, int z) {
    if (dp[curr_pos][z][flag] != -1) return dp[curr_pos][z][flag];
    if (k == 0) return a[curr_pos];
    else if (z == 0 || curr_pos == 0 || !flag)
        dp[curr_pos][z][flag] = a[curr_pos] + maxScore(n, curr_pos + 1, k - 1, true, z);
    else {
        dp[curr_pos][z][flag] = a[curr_pos] + max(maxScore(n, curr_pos - 1, k - 1, false, z - 1),
                                                  maxScore(n, curr_pos + 1, k - 1, true, z));
    }
    return dp[curr_pos][z][flag];
}
    ...
    while (t--) {
        ...
        for (int i = 0; i < n + 33; i++)
            for (int j = 0; j < 5; j++)
                for (int x = 0; x < 2; x++)
                    dp[i][j][x] = -1;
        cout << maxScore(n, 0, k, true, z) << endl;
    }
}