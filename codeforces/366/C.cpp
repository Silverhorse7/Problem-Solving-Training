...
int solve(int i, int rem, int f = 0) {
    if (i >= n) {
        if (!rem and f)return 0;
        return INT_MIN;
    }
    if (~dp1[i][rem])return dp1[i][rem];
    return dp1[i][rem] = max(solve(i + 1, rem, f),
                             solve(i + 1, rem + a[i] - k * b[i], 1) + a[i]
    );
}
