...
signed main() {
    ...
    function<double(int, int)> solve = [&](int i, int time) {
        if (i >= n or time == 0)return i * 1.0;
        if (dp[i][time] > -0.5)return dp[i][time];
        dp[i][time] = 0;
        return dp[i][time] = p * solve(i + 1, time - 1)
                             +
                             (1.0 - p) * solve(i, time - 1);

    };
    cout << fixed << setprecision(6) << solve(0, t);
}