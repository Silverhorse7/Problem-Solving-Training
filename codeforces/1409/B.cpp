...
// remove the ceil macro
{
	...
	cin >> a >> b >> x >> y >> n;
	// a>=x , b>=y , n times
	// we have two cases , lma ttr7 kol n mn rakm wa7d
	// other case lma al tar7 msh kafya
//	int f = max(n - a + x, 0);
	ll ff = 0LL;
	ll first = (a - min(n, a - x)) * (b - min(max(n - a + x,ff), b - y));
	ll second= (b - min(n, b - y)) * (a - min(max(n - b + y,ff), a - x));;
